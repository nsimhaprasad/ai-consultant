...
class BaidToolWindowView(
    ...
    private val mainPanel: JBPanel<JBPanel<*>?> = JBPanel<JBPanel<*>?>(BorderLayout())
    ...
    init {
        this.loginPanel = LoginPanelView(project, authController, { userProfile -> userProfile?.let { actions.loginSuccess(it) } }, { e -> actions.loginError(e) })
        this.chatPanel = ChatPanelView(project) // Pass authController
        this.pastConversationsView = PastConversationsView(project, sessionController) { sessionPreview -> sessionPreview?.let { actions.selectSession(it) } }
